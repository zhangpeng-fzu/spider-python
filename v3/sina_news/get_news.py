            ...
            for news in response_list["result"]["data"]:
                ...
                try:
                    with connection.cursor() as cursor:
                        sql = 'INSERT INTO news VALUES (%s,%s,%s,%s) ON DUPLICATE KEY UPDATE TITLE = %s,URL = %s,CREATE_TIME=%s'
                        cursor.execute(sql, (news_id, title, link, create_time, title, link, create_time))
                    connection.commit()
