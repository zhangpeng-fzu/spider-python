...
class TM_producs(object):
    ...
    def init_mysql(self,conn): 
        ...
        cur.execute("create table phone_info(id varchar(50) ,shop_id varchar(50) ,title varchar(100),url varchar(100),img varchar(100),sold varchar(100),totalSoldQuantity varchar(100),price varchar(100))")
        cur.close()
        return
    ...
    def save_common_info(self,data):
        ...
        print(result)
        if result == None:
            cur.execute("insert into shop_info values('"+data.get('shop_id')+"','"+data.get('shop_title')+"','https:"+data.get('shop_Url')+"')") 
    ...
    def save_phone_info(self,data):
        ...
        for row in products: 
            #print(row)
            cur.execute("insert into phone_info values('"+str(row.get('item_id'))+"','"+str(data.get('shop_id'))+"','"+row.get('title')+"','https:"+row.get('url')+"','"+row.get('img')+"','"+str(row.get('sold'))+"','"+str(row.get('totalSoldQuantity'))+"','"+str(row.get('price'))+"')") 
        cur.close()
        conn.commit()
        conn.close()
        return
    ...
    def save_rom_info(self,data,rom,pid,item,typeId):
        ...
        for row in rom.get('values'):
            ...
            rom = row.get('name')
            cur.execute("insert into rom_info values('"+str(item.get('itemId'))+"','"+str(rom)+"','"+str(price)+"')") 
        cur.close()
        conn.commit()
        conn.close()
    ...
    def main(self):
        ...
        temp = [
            ...
            {'shop_name':'xiaomi','key':'%E5%B0%8F%E7%B1%B3%20%E7%BA%A2%E7%B1%B3%E6%89%8B%E6%9C%BA','sid':758144452},
