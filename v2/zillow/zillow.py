...
head = {
    ...
    'method': 'GET',
    'path': '/homes/recently_sold/Ladera-Heights-CA-90230/house,condo,apartment_duplex,townhouse_type/96095_rid/12m_days/globalrelevanceex_sort/34.036017,-118.320694,33.958169,-118.473301_rect/12_zm/',
    'scheme': 'https'
}
...
def get_zpid(url):
    ...
    try:
        ...
        results = re.findall(r"homedetails.*?_zpid", content, re.I | re.S | re.M)
        for result in results:
            zpid.append(result.split("/")[2].replace("_zpid", ""))
        return zpid
    except Exception, e:
        print e
        return zpid
...
for i in range(14):
    try:
        ...
        url = "https://www.zillow.com/homes/recently_sold/Ladera-Heights-CA-90230/house,condo,apartment_duplex,townhouse_type/96095_rid/12m_days/globalrelevanceex_sort/34.036017,-118.320694,33.958169,-118.473301_rect/12_zm/%s_p/" % (
            i + 1)
        zpid = get_zpid(url)
        print "获取第" + str(i + 1) + "页数据完成"
        zpids = zpids + zpid
    except Exception, e:
        print e

print "一共获取" + str(len(zpids)) + "条数据。zips=" + str(zpids)
